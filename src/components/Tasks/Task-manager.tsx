...
const TaskManager = () => {
  ...
  const handleUpdateTask = async (updatedTask: Task) => {
    try {
      const response = await axios.put(`http://localhost:5500/api/tasks/${updatedTask._id}`, updatedTask, {
        headers: {
          Authorization: `${accessToken}`,
        },
      });
  ...
  const handleCreateTask = async (newTask: any) => {
    try {
      const response = await axios.post('http://localhost:5500/api/tasks', newTask, {
        headers: {
          Authorization: `${accessToken}`,
        },
      });
